...
import { getResumeService, updateResumeService, deleteResumeService, downloadResumeService, } from "./resume.service.js";
export const getResumeController = async (req, res) => {
    try {
        ...
        logger.error(`getResumeController: Exception occurred - ${error instanceof Error ? error.message : String(error)}`);
...
export const updateResumeController = async (req, res) => {
    try {
        ...
        if (!fileName || !fileUrl || !fileSize || !publicId) {
            logger.warn(`updateResumeController: Missing required fields for user ${userId}`);
            return res.status(400).json({
                success: false,
                message: "Missing required fields - fileName, fileUrl, fileSize, publicId",
                data: null,
            });
        }
        logger.info(`updateResumeController: Attempting to update resume for user ${userId}`);
        ...
        if (!success) {
            logger.error(`updateResumeController: Failed to update resume for user ${userId} - ${error}`);
            return res.status(400).json({
                success: false,
        ...
        logger.info(`updateResumeController: Resume updated successfully for user ${userId}`);
        return res.status(200).json({
            success: true,
        ...
        logger.error(`updateResumeController: Exception occurred - ${error instanceof Error ? error.message : String(error)}`);
        return res.status(500).json({
            success: false,
...
export const deleteResumeController = async (req, res) => {
    try {
        ...
        logger.info(`deleteResumeController: Attempting to delete resume for user ${userId}`);
        const { success, error } = await deleteResumeService(userId);
        if (!success) {
            logger.error(`deleteResumeController: Failed to delete resume for user ${userId} - ${error}`);
            return res.status(400).json({
                success: false,
        ...
        logger.info(`deleteResumeController: Resume deleted successfully for user ${userId}`);
        return res.status(200).json({
            success: true,
        ...
        logger.error(`deleteResumeController: Exception occurred - ${error instanceof Error ? error.message : String(error)}`);
        return res.status(500).json({
            success: false,
...
export const downloadResumeController = async (req, res) => {
    try {
        ...
        logger.info(`downloadResumeController: Attempting to download resume for user ${userId}`);
        const { success, data, error } = await downloadResumeService(userId);
        if (!success || !data) {
            logger.error(`downloadResumeController: Failed to retrieve resume for user ${userId} - ${error}`);
