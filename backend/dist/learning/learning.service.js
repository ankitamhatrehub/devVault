...
export const createLearningService = async (userId, learningData) => {
    try {
...
export const getAllLearningsService = async (userId) => {
    try {
...
export const getLearningByIdService = async (learningId, userId) => {
    try {
...
export const updateLearningService = async (learningId, userId, updateData) => {
    try {
        logger.info("Updating learning roadmap => " + learningId + " for user => " + userId);
        const learning = await LearningModel.findOneAndUpdate({ _id: learningId, userId, deletedAt: null }, updateData, { new: true });
        if (!learning) {
...
export const deleteLearningService = async (learningId, userId) => {
    try {
        logger.info("Deleting learning roadmap => " + learningId + " for user => " + userId);
        const learning = await LearningModel.findOneAndUpdate({ _id: learningId, userId, deletedAt: null }, { deletedAt: new Date() }, { new: true });
        if (!learning) {
