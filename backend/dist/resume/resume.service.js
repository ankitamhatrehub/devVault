...
export const getResumeService = async (userId) => {
    try {
        ...
        logger.info(`getResumeService: Resume fetched successfully for user ${userId}`);
        return resume;
    }
    catch (error) {
        logger.error(`getResumeService: Error fetching resume for user ${userId} - ${error instanceof Error ? error.message : String(error)}`);
        throw Error(`Failed to fetch resume: ${error instanceof Error ? error.message : String(error)}`);
    }
};
export const updateResumeService = async (userId, payload) => {
    try {
        ...
        const updatedResume = await ResumeSchema.findByIdAndUpdate(existingResume._id, {
            fileName: payload.fileName,
            fileUrl: payload.fileUrl,
            fileSize: payload.fileSize,
            publicId: payload.publicId,
        }, { new: true, runValidators: true });
        logger.info(`updateResumeService: Resume updated successfully for user ${userId}`);
        return {
            success: true,
            data: updatedResume,
        };
        ...
        logger.error(`updateResumeService: Error updating resume for user ${userId} - ${error instanceof Error ? error.message : String(error)}`);
        return {
            success: false,
...
export const deleteResumeService = async (userId) => {
    try {
        ...
        logger.info(`deleteResumeService: Resume deleted successfully for user ${userId}`);
        return {
            success: true,
        };
    }
    catch (error) {
        logger.error(`deleteResumeService: Error deleting resume for user ${userId} - ${error instanceof Error ? error.message : String(error)}`);
        return {
            success: false,
...
export const downloadResumeService = async (userId) => {
    try {
        ...
        logger.info(`downloadResumeService: Resume retrieved successfully for user ${userId}`);
        return {
            success: true,
            data: resume,
        };
        ...
        logger.error(`downloadResumeService: Error retrieving resume for user ${userId} - ${error instanceof Error ? error.message : String(error)}`);
        return {
            success: false,
